...
Лёгкий watchdog для Yandex Cloud, который живёт на VPS, сам поднимается после ребута и через красивую CLI-панель следит за вашими VM.
...
`yandex-auto-up` запускается на маленьком VPS и делает ровно одну работу: следит за выбранными VM в Yandex Cloud и поднимает их, когда они должны быть онлайн. Без Docker в рантайме, без тяжёлой TUI, без разрастания фоновых процессов.

> [!IMPORTANT]
> Проект может запускать платные облачные VM. Перед боевым использованием проверьте квоты, стоимость, IAM-права и лимиты Yandex Cloud.
...
- Отправляет команду запуска в Yandex Cloud, если инстанс остановлен.
...
3. Скопируйте один или несколько файлов ключей в `/etc/yandex-auto-up/keys/`.
4. Импортируйте VM из облака или создайте профиль вручную.
5. Проверьте статус через `yauto status`.

Каталог `keys/` создаётся автоматически с файлом-подсказкой. Все файлы в этой папке проверяются по **содержимому** (не по расширению) — любое имя файла и любое количество.
